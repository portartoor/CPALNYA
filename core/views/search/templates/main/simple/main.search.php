...
?>
<style>
.search-shell{max-width:1240px;margin:0 auto;padding:24px 16px 44px;color:var(--shell-text);font-family:"Sora",system-ui,sans-serif}
.search-hero,.search-featured,.search-card,.search-empty,.search-form{border:1px solid var(--shell-border);background:var(--shell-panel);backdrop-filter:blur(14px);box-shadow:var(--shell-shadow)}
.search-hero{padding:28px;border-radius:24px;display:grid;gap:14px}
...
.search-hero h1{font-size:clamp(2rem,4vw,3.6rem);line-height:.92}
.search-hero p,.search-card p,.search-featured-copy p,.search-empty,.search-meta,.search-results-head{color:var(--shell-muted)}
.search-form{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;padding:10px;border-radius:16px}
...
.search-featured{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(280px,.85fr);gap:18px;padding:18px;border-radius:22px}
...
.search-card{display:grid;gap:12px;padding:14px;border-radius:18px}
...
.search-empty{margin-top:18px;padding:18px;border-radius:20px}
...
@media (max-width:680px){.search-shell{padding:18px 12px 34px}.search-form{grid-template-columns:1fr}.search-grid{grid-template-columns:1fr}.search-featured{padding:14px}.search-featured-media{min-height:220px}}
</style>

<section class="search-shell">
    <div class="search-hero">
        <h1><?= $escape($isRu ? 'Поиск по выпуску' : 'Search the issue') ?></h1>
        <p><?= $escape($isRu ? 'Ищем по журналу, практике, повестке и фан-блокам. Если точного попадания нет, не оставляем пустоту, а подкидываем живые материалы для захода дальше.' : 'Search across journal, playbooks, signals and fun. If there is no exact hit, we still surface strong material worth opening next.') ?></p>
            ...
            <div class="search-grid">
                <?php foreach (array_slice($items, 1) as $item): ?>
                    <article class="search-card">
                        ...
                        <span class="search-meta"><?= $escape($sectionLabel((array)$item, $isRu)) ?></span>
    ...
    <?php else: ?>
        <div class="search-empty">
            <?= $escape($isRu ? ('По запросу "' . $query . '" точных совпадений нет. Ниже — один материал пошире и еще несколько случайных заходов, чтобы не упираться в пустую выдачу.') : ('No exact matches for "' . $query . '". Here is one broader article plus a few random ways in.')) ?>
        </div>
