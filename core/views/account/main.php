...
.pacc-hero h1{margin:0 0 10px;font:700 clamp(1.65rem,2.4vw,2.2rem)/.98 "Space Grotesk","Sora",sans-serif;letter-spacing:-.05em}
.pacc-card h2,.pacc-auth h2{margin:0 0 10px;font:700 clamp(1rem,1.45vw,1.2rem)/1.02 "Space Grotesk","Sora",sans-serif;letter-spacing:-.035em}
.pacc-card h3{margin:0 0 10px;font:700 1rem/1.05 "Space Grotesk","Sora",sans-serif;letter-spacing:-.03em}
...
.pacc-grid{display:grid;grid-template-columns:minmax(280px,.8fr) minmax(0,1.2fr);gap:18px}
...
.pacc-mini a{color:var(--shell-accent);text-decoration:none}
...
<section class="pacc">
    <div class="pacc-shell">
        <header class="pacc-hero">
            <h1><?= htmlspecialchars($publicProfile ? $t('Профиль участника обсуждения', 'Member profile') : $t('Профиль сообщества', 'Community profile'), ENT_QUOTES, 'UTF-8') ?></h1>
            <p><?= htmlspecialchars($publicProfile ? $t('Открытая карточка участника: как его читают, поддерживают и где он уже успел оставить заметные реплики под материалами.', 'An open member card: how the community reads, supports and sees this author across article discussions.') : $t('Здесь видна ваша открытая карточка в редакционном обсуждении: имя, рейтинг, звание и опубликованные комментарии.', 'This page shows your public card in the editorial discussion: name, rating, title and published comments.'), ENT_QUOTES, 'UTF-8') ?></p>
        </header>
        ...
        <?php if ($publicProfile): ?>
            <section class="pacc-card">
                <div class="pacc-grid">
                    ...
                    <section class="pacc-card">
                        <h2><?= htmlspecialchars($t('Публичная карточка автора', 'Public author card'), ENT_QUOTES, 'UTF-8') ?></h2>
                        <div class="pacc-rank">
                            ...
                            <div class="pacc-rank-note"><?= htmlspecialchars($t('Здесь остается только то, что человек решил вынести в открытую часть профиля: репутация в обсуждениях, заметные следы под материалами и опубликованные ссылки.', 'This keeps only what the member chose to make public: discussion reputation, visible traces under articles and published links.'), ENT_QUOTES, 'UTF-8') ?></div>
                            <div class="pacc-mini">
            ...
            <section class="pacc-card">
                <h2><?= htmlspecialchars($t('Последние реплики', 'Latest replies'), ENT_QUOTES, 'UTF-8') ?></h2>
                <div class="pacc-recent">
                    ...
                    <?php if (empty($publicProfile['recent_comments'])): ?>
                        <p><?= htmlspecialchars($t('У этого участника пока нет открытых комментариев.', 'This member has no public comments yet.'), ENT_QUOTES, 'UTF-8') ?></p>
                    <?php endif; ?>
                </div>
            </section>
        <?php elseif ($user): ?>
            <div class="pacc-grid">
                ...
                <section class="pacc-card">
                    <h2><?= htmlspecialchars($t('Ваша открытая карточка', 'Your public card'), ENT_QUOTES, 'UTF-8') ?></h2>
                    <?php if ($ownRank): ?>
                        <div class="pacc-rank">
                            <span class="pacc-rank-badge"><?= htmlspecialchars((string)($ownRank['label'] ?? ''), ENT_QUOTES, 'UTF-8') ?></span>
                            <div class="pacc-rank-note">
                                <?= htmlspecialchars($t('Здесь остается только публичная часть: имя, рейтинг, звание и след ваших комментариев под материалами.', 'Only the public-facing part stays here: your name, rating, title and the visible trail of your comments under articles.'), ENT_QUOTES, 'UTF-8') ?>
                            ...
                            <div class="pacc-mini">
                                ...
                                <span><a href="<?= htmlspecialchars(function_exists('public_portal_profile_url') ? public_portal_profile_url($user) : '/member/', ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($t('Открыть публичную страницу', 'Open public page'), ENT_QUOTES, 'UTF-8') ?></a></span>
                            </div>
                        </div>
                    <?php endif; ?>
                    <div class="pacc-mini">
                        <?php if (!empty($user['telegram_handle'])): ?><span>Telegram: <?= htmlspecialchars((string)$user['telegram_handle'], ENT_QUOTES, 'UTF-8') ?></span><?php endif; ?>
                        <?php if (!empty($user['website_url'])): ?><span><a href="<?= htmlspecialchars((string)$user['website_url'], ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="nofollow noopener"><?= htmlspecialchars($t('Сайт', 'Website'), ENT_QUOTES, 'UTF-8') ?></a></span><?php endif; ?>
                        <?php if (!empty($user['email'])): ?><span><?= htmlspecialchars((string)$user['email'], ENT_QUOTES, 'UTF-8') ?></span><?php endif; ?>
                    </div>
                </section>
            </div>
            <section class="pacc-card">
                <h2><?= htmlspecialchars($t('Ваши последние реплики', 'Your latest replies'), ENT_QUOTES, 'UTF-8') ?></h2>
                <div class="pacc-recent">
