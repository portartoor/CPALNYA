    ...
    <div class="jrnl-shell">
        <?php if ($selected): ?>
            <article class="jrnl-detail">
                <span class="jrnl-kicker"><?= htmlspecialchars((string)($issue['issue_title'] ?? $t('HowTo / Playbooks', 'HowTo / Playbooks')), ENT_QUOTES, 'UTF-8') ?></span>
                ...
                <div class="jrnl-detail-body">
                    ...
                    <div class="jrnl-actions">
                        <a class="jrnl-btn" href="<?= htmlspecialchars($buildPageUrl($currentCluster), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($t('Назад в playbooks', 'Back to playbooks'), ENT_QUOTES, 'UTF-8') ?></a>
                    </div>
                </div>
            </article>
        ...
        <?php else: ?>
            <header class="jrnl-hero">
                <div class="jrnl-copy">
                    ...
                    <h1><?= htmlspecialchars((string)($issue['hero_title'] ?? $issue['issue_title'] ?? $t('HowTo / Playbooks', 'HowTo / Playbooks')), ENT_QUOTES, 'UTF-8') ?></h1>
            ...
            <?php else: ?>
                <div class="jrnl-empty">
                    ...
                    <p><?= htmlspecialchars($t('Когда статьи появятся, они будут выведены здесь как отдельный раздел playbooks с фильтрацией по темам.', 'Published articles will appear here as a dedicated playbooks section with topic filters.'), ENT_QUOTES, 'UTF-8') ?></p>
                </div>
            <?php endif; ?>
        <?php endif; ?>
    </div>
