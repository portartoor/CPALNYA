<div class="container-fluid mt-4">
    ...
    <div class="card mb-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <a class="btn btn-primary btn-sm" href="/adminpanel/services-edit/">Create Service</a>
        </div>
        <div class="card-body">
            <form method="get" class="row g-2 align-items-end">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="group" class="form-control" value="<?= htmlspecialchars((string)$filterGroup) ?>" placeholder="group code">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Search</label>
                    <input type="text" name="q" class="form-control" value="<?= htmlspecialchars((string)$q) ?>" placeholder="title or slug">
                </div>
                <div class="col-md-2">
            ...
            <?php if (!empty($groupOptions)): ?>
                <div class="mt-3 d-flex flex-wrap gap-2">
                    <?php foreach ($groupOptions as $opt): ?>
                        <span class="badge text-bg-light"><?= htmlspecialchars((string)$opt['code']) ?> (<?= (int)$opt['count'] ?>)</span>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive">
            <table class="table table-striped align-middle">
                ...
                <tbody>
                <?php if (!empty($servicesRows)): foreach ($servicesRows as $row): ?>
                    <tr>
                        <td><?= (int)$row['id'] ?></td>
                        <td><span class="badge text-bg-secondary"><?= htmlspecialchars((string)$row['lang_code']) ?></span></td>
                        <td><?= htmlspecialchars((string)$row['service_group']) ?></td>
                        ...
                        <td class="d-flex gap-1 flex-wrap">
                            <a class="btn btn-sm btn-outline-primary" href="/adminpanel/services-edit/?id=<?= (int)$row['id'] ?>">Edit</a>
                ...
                <?php endforeach; else: ?>
                    <tr><td colspan="10" class="text-muted">No services found.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
        <div class="card-footer d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2">
                <?php
                    $qs = 'lang=' . urlencode((string)$filterLang) . '&group=' . urlencode((string)$filterGroup) . '&q=' . urlencode((string)$q) . '&per_page=' . (int)$perPage;
