...
function adminpanel_section_key_from_path(string $path): string
{
    ...
    if (strpos($path, '/blog') === 0 || strpos($path, '/examples/article/') === 0 || strpos($path, '/articles/') === 0) {
        return 'blog';
    }
...
$articlePathWhereSql = "(
    (path LIKE '/blog/%' AND path NOT IN ('/blog', '/blog/'))
    OR
...
$sectionCaseSql = "
    CASE
        ...
        WHEN path LIKE '/blog%' OR path LIKE '/examples/article/%' OR path LIKE '/articles/%' THEN 'blog'
